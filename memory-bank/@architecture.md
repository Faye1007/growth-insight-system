...
- `src/components/app-shell.tsx`: 共享应用壳，负责左侧或顶部主导航、导航图标、品牌区、当前阶段提示、账号状态和退出入口，并把页面内容包裹在统一布局中；当前主导航保留成长主页、每日工作台、成长记录、洞察报告、纪念日和设置，不再展示个人说明书、问题拆解和 Markdown 导出独立入口。
...
- `/insights`: 洞察报告。
- `/manual`: 个人说明书，已支持登录用户读取、首次保存和完整字段手动编辑；当前不在主导航展示，后续并入洞察报告的 AI 复盘区域。
