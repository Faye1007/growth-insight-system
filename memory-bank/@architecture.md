...
- `src/app/settings/page.tsx`: 公开版设置页，改名为"账号与应用设置"；未登录状态显示"登录/注册"按钮；已登录状态展示当前账号登录状态、昵称编辑（通过 Supabase Auth `updateUser()` 更新 `user_metadata`）和账号注销功能（二次确认后软删除所有业务数据并退出登录）；展示功能可用性（任务管理、习惯打卡、日程记录、随手记录、洞察报告与图表、AI 复盘）；新增"关于 AI 复盘"说明，明确 AI 是可选增强，未配置时不影响普通功能；不展示任何密钥、连接字符串或底层错误。
