...
- 登录后写入保护 helper。
- 侧边栏账号状态展示。
- 移动端底部 5 Tab 导航，入口为清单、人生、AI、复盘和设置；桌面端保留侧边栏/顶部账号入口。
- 每日工作台入口页，包括顶部日期、今日概览和晚间总结入口；默认不再直接铺开任务、习惯、日程和随手记录列表。
- 每日工作台未登录写入拦截提示。
...
- 延期任务日期同步更新和延期来源记录。
- 今日概览展示任务完成率、习惯打卡、今日日程和随手记录完成情况；四张概览卡可点击，分别跳转清单页任务、习惯、日程 tab 和人生页事件 tab。
- 每日工作台习惯创建表单。
- 习惯保存 Server Action。
...
- 今日完成情况已从洞察报告合并回每日工作台今日概览，用户先看完成情况，再通过卡片进入清单或人生列表。
- `/daily?view=...` 旧入口仍保留，用于清单页新增按钮打开对应创建表单；默认 `/daily` 不展示当前列表。
- 每日工作台手机宽度下已优化卡片内边距、状态标签换行、列表操作区、延期日期输入、编辑表单按钮和复盘操作区，避免常见横向溢出和按钮拥挤。
- 成长记录页读取当前登录用户近期任务、习惯打卡、日程、事件和灵感，并按创建时间倒序合并为统一时间线后按北京时间日期分组展示。
- 成长记录页按任务、习惯、日程、事件和灵感统计当前载入数量；移动端统计概览使用三列紧凑网格。
- 成长记录页支持通过 URL query 参数按记录类型和日期范围筛选。
- 成长记录页事件条目最多展示两行内容，完整内容进入事件详情页查看和编辑。
- 成长记录时间线条目可以进入详情页。
...
- 设置页只展示用户可理解的账号信息、功能可用性和 AI 复盘说明；不展示 Supabase、数据库连接、AI key 等内部配置字段。
...
### 1.0.1 Mobile-First Refactor（Step 19，已完成）

系统已完成移动端优先的底部 5 Tab 导航重构：
...
- `src/app/life/page.tsx`: 人生页面，当前包含事件、纪念日和礼物三个 tab；登录用户读取自己的事件、纪念日列表和礼物列表，事件、纪念日和礼物三组数据用 `Promise.allSettled` 分组加载，单组失败时显示统一提示但不拖垮整页；列表展示由 `LifeClient` 负责，按日期分组展示，未登录用户只看到页面结构和登录提示。
- `src/app/life/[kind]/[id]/page.tsx`: 人生详情页，当前支持 `anniversary` 和 `gift` 两类详情；纪念日详情支持查看、编辑、软删除，并直接展示关联的历史礼物记录列表，礼物标题可进入礼物详情；礼物详情支持查看、编辑、软删除和选择关联纪念日。
- `src/app/life/actions.ts`: 生活扩展 Server Actions，当前提供 `createAnniversaryAction()`、`updateAnniversaryAction()`、`softDeleteAnniversaryAction()`、`createGiftRecordAction()`、`updateGiftRecordAction()` 和 `softDeleteGiftRecordAction()`；写入前必须通过 `requireCurrentUser()` 获取当前用户，并按当前 `user_id` 限定创建、更新和软删除范围；更新和删除 action 支持从详情页返回对应详情或回到人生页。
...
- `src/app/settings/page.tsx`: 公开版设置页，改名为"账号与应用设置"；未登录状态显示"登录/注册"按钮；已登录状态展示账号摘要、邮箱、昵称、登录状态、退出登录、清除数据和账号注销；昵称作为主标题展示，点击昵称旁边的笔图标展开编辑，通过 Supabase Auth `updateUser()` 更新 `user_metadata`；清除数据会保留账号并物理删除所有业务数据，注销账号会先物理删除业务数据再删除 Supabase Auth 用户；注销账号仍由客户端确认按钮二次确认，且服务端未配置 `SUPABASE_SERVICE_ROLE_KEY` 时会提示缺少权限而不清除数据；展示功能可用性（任务管理、习惯打卡、日程记录、随手记录、洞察报告与图表、AI 复盘）；新增"关于 AI 复盘"说明，明确 AI 是可选增强，未配置时不影响普通功能；不展示任何密钥、连接字符串或底层错误。
- `src/app/login/page.tsx`: 邮箱登录和注册页面，支持 `next` 参数把用户带回原页面。
- `src/app/auth/actions.ts`: Supabase Auth Server Actions，负责登录、注册、退出、昵称更新、账号数据清除和账号注销；账号数据清除使用当前用户会话物理删除业务表数据但保留 Auth 用户，账号注销要求 `SUPABASE_SERVICE_ROLE_KEY`，通过 Supabase Admin API 删除 Auth 用户。
- `src/app/auth/confirm/route.ts`: Supabase 邮箱确认回调路由，成功后跳转到安全的 `next` 路径，失败时回到登录页。
- `src/app/globals.css`: 全局样式入口，导入 Tailwind CSS，定义基础视觉 token、字体、页面标题、卡片、列表、状态标签、基础按钮、导航样式、底部导航、每日概览卡片入口、设置页账号摘要、单行/两行记录预览、洞察报告统计网格、洞察报告趋势卡、任务完成率图表容器、习惯打卡图表容器和点阵、记录数量趋势图表容器、情绪基础统计图表容器、成长记录分组列表、记录概览统计、成长记录筛选控件、记录详情字段、工作台面板、空状态、创建区收起面板、任务表单、任务编辑表单、删除危险区、任务列表、任务状态分组、状态操作按钮、延期日期输入、习惯统计标签行、每日复盘预览区、每日复盘报告区、文本域、表单提示和移动端适配样式。
- `src/components/app-shell.tsx`: 共享应用壳，负责左侧或顶部主导航、导航图标、品牌区，并把页面内容包裹在统一布局中；PC 端顶部横条右上角未登录时显示登录/注册，已登录时只显示昵称并跳转 `/settings`，账号状态、退出登录和昵称编辑集中放到设置页；左侧导航只保留产品标识和主导航入口，不再展示内部阶段卡片和账号状态卡片；当前主导航保留清单、人生、AI、复盘和设置，不再展示成长主页、个人说明书、问题拆解和 Markdown 导出独立入口。
- `src/components/bottom-nav.tsx`: 移动端底部 5 Tab 导航客户端组件，包含清单、人生、AI、复盘和设置入口，并根据当前路径高亮。
...
- `drizzle/0012_remove_schedule_date.sql`: 移除 `schedule_date` 冗余字段迁移 SQL，删除 `schedule_items` 的 `schedule_date` 列及其索引；`start_date` 改为 `notNull`，成为日程的唯一日期字段；所有查询统一使用 `start_date + recurrence` 规则判断日程是否命中某天。
...
- 已建立每日工作台入口页，今日任务创建、任务状态更新、习惯创建、习惯打卡、习惯编辑与停用、今日日程记录和随手记录能力仍保留在 `/daily?view=...` 旧入口中，供清单页新增按钮调用。
- 每日工作台的任务、习惯、日程和随手记录创建表单默认收起，用户从清单页新增入口跳转后展开；保存失败时对应表单自动展开并显示统一反馈。
- 今日概览四张卡已改为入口卡片，默认跳转清单页任务/习惯/日程 tab 或人生页事件 tab，不再直接创建内容。
...
- `SUPABASE_SERVICE_ROLE_KEY` 仅用于账号注销这类服务端管理能力；未配置时普通记录、统计、清除数据和 AI 以外功能不受影响，但不能删除 Supabase Auth 用户。
...
- `/life`: 人生页，已支持事件、纪念日和礼物按日期分组列表、快捷新增入口、事件详情跳转、纪念日和礼物详情跳转。
- `/life/[kind]/[id]`: 人生详情页，当前支持纪念日和礼物详情、编辑和软删除。
...
- 设置页账号摘要、昵称编辑、退出登录、清除数据、账号注销、功能可用性和 AI 复盘说明展示；不再对用户展示数据库健康检查或内部配置状态。
...
- `SUPABASE_SERVICE_ROLE_KEY` 只作为账号注销的服务端管理权限使用；具体是否配置取决于部署环境，未配置时注销账号会提示缺少权限。
...
- 使用 `src/app/auth/actions.ts` 执行登录、注册、退出、昵称更新、账号数据清除和账号注销。
...
### 6.13 Excel Historical Data Import

`Faye的成长计划.xlsx` 中从 Coze + 飞书多维表格沉淀的历史数据已通过一次性脚本导入当前系统。

导入目标：

- 账号：`2215128728@qq.com`
- `user_id`: `98779cb3-d200-4a9f-9b84-7ea206f89e0d`
...
- 所有导入数据均写入目标账号 `user_id` 下。
- 飞书任务 ID、飞书日历事件 ID 只在导入期间用于临时关联，不写入业务表。
- 固定日程中已停用数据因当前 `schedule_items` 无 `is_active` 字段而跳过。
- 空灵感和空人生笔记跳过。
- Node 22 + `postgres` 在连续写入时出现过 `CONNECTION_CLOSED`，导入脚本已改为每条记录新连接的保守模式；数据量小，后续如需重跑可依赖自然键查重避免重复导入。
