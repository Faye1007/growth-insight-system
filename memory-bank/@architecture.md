...
- `src/app/settings/page.tsx`: 公开版设置页，改名为"账号与应用设置"；未登录状态显示"登录/注册"按钮；已登录状态展示账号摘要、邮箱、昵称、登录状态、退出登录和账号注销；昵称作为主标题展示，点击昵称旁边的笔图标展开编辑，通过 Supabase Auth `updateUser()` 更新 `user_metadata`；退出登录和注销账号并列显示，注销账号仍由客户端确认按钮二次确认；展示功能可用性（任务管理、习惯打卡、日程记录、随手记录、洞察报告与图表、AI 复盘）；新增"关于 AI 复盘"说明，明确 AI 是可选增强，未配置时不影响普通功能；不展示任何密钥、连接字符串或底层错误。
...
- 设置页账号摘要、昵称编辑、退出登录、账号注销、功能可用性和 AI 复盘说明展示；不再对用户展示数据库健康检查或内部配置状态。
