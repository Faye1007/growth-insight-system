...
- `src/app/settings/page.tsx`: 设置页展示应用运行状态、账号登录状态、Supabase 配置状态、数据库只读健康检查结果和 AI 配置状态；数据库健康检查使用独立短连接执行 `select 1`，成功时显示连接正常，失败时显示连接异常；只显示是否配置和连接状态，不展示密钥、token、API key、连接字符串或底层错误堆栈。
