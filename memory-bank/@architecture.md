...
- `src/app/life/page.tsx`: 人生页面，当前包含事件、纪念日和礼物三个 tab；登录用户读取自己的事件、纪念日列表和礼物列表，事件、纪念日和礼物三组数据用 `Promise.allSettled` 分组加载，单组失败时显示统一提示但不拖垮整页；未登录用户只看到页面结构和登录提示。
