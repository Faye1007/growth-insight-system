...
### ✅ Modification Step 25.6：循环日程按日期打卡

完成内容：

- 新建 `schedule_completions` 表（`schedule_id + completion_date` 唯一约束），用于按日期记录日程完成状态。
- 新建数据库迁移文件 `drizzle/0011_schedule_completions.sql`，包含表结构、索引、RLS 策略和 Supabase JS 授权。
- `src/db/schema.ts` 新增 `scheduleCompletions` 表定义。
- `user-data.ts` 新增 `upsertScheduleCompletionForUser`、`deleteScheduleCompletionForUser`、`getScheduleCompletionsForUser` 三个函数。
- `getChecklistSchedulesForUser` 不再返回 `isCompleted` 字段，改为由 `getScheduleCompletionsForUser` 提供按日期的完成状态 Map。
- `ChecklistSchedule` 类型移除 `isCompleted` 字段。
- `toggleScheduleCompletionAction` 改为 upsert/delete `schedule_completions` 表记录，不再更新 `schedule_items.is_completed`。
...
### ✅ Modification Step 25.5：成长记录列表完成状态改为复选框+颜色

完成内容：

- `TimelineRecord` 类型增加 `status` 和 `isCompleted` 字段。
- 任务记录不再显示文字状态标签（已完成、未开始等），改为通过 `isCompleted` 判断显示复选框图标和颜色。
- 习惯打卡不再显示"已打卡"/"已取消"文字标签，改为复选框图标。
- 日程记录增加 `isCompleted` 字段，`getRecentScheduleItemsForUser` 查询增加 `is_completed` 列。
- `RecentScheduleRecord` 类型增加 `isCompleted` 字段。
- 已完成的任务/习惯/日程在成长记录中显示删除线和 `task-status-completed` 颜色。
- 移除不再使用的 `getTaskStatusLabel` 导入和 `habitCheckinStatusLabels` 常量。

验证：

- `npm run build` 通过。

### ✅ Modification Step 25.4：清单日程只显示今天及之前的日程

完成内容：

- 清单日程列表过滤条件改为 `scheduleDate <= todayStr`，不再显示未来日程。
- 非循环日程只展示 `scheduleDate` 在今天及之前的记录。
- 循环日程保持原有逻辑（在有效期内始终展示）。
- 未来日程需到成长记录页查看。
...
### ✅ Modification Step 25.3：延期任务专区

完成内容：

- 新增 `getPostponedTasksForUser` 数据函数，查询所有 `is_postponed = true` 且 `status != completed` 的任务，按 `postponed_to_date` 排序。
- 清单页服务端并行加载延期任务数据。
- 清单任务列表顶部增加"延期任务"专区，使用 `text-[var(--clay)]` 颜色标识。
- 延期任务支持复选框操作，完成后从专区消失。
- 延期任务 meta 信息显示分类和延期目标日期。

验证：

- `npm run build` 通过。

### ✅ Modification Step 25.2：任务标注完成不跳转

完成内容：

- `updateTaskStatusAction` 增加 `source` 参数读取。
- 当 `source === "checklist"` 时，所有错误和成功分支重定向到 `/checklist?tab=tasks`，保持在清单页面。
- 默认行为不变，仍重定向到 `/daily`。
- 清单页任务状态操作 form 增加 `<input type="hidden" name="source" value="checklist" />`。
...
### ✅ Modification Step 25.1：周视图日期格式精简

完成内容：

- 清单页周视图日期范围从 `2026-05-18 ~ 2026-05-24` 改为 `05-18 ~ 05-24`。
- 新增 `formatShortDate()` 工具函数，截取日期字符串的 MM-DD 部分。
...
下一阶段：**Modification Step 26** 已规划，包含 11 项改进，按优先级逐步实施。第一步（Step 26.1）为重复工具函数提取。
