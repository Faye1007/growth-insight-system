...
当前项目已经完成从 0 到 1 的基础闭环、真实数据导入、部署基础验收，以及多轮真实使用后的体验改造。
...
- 全局搜索新增覆盖 `schedule_items.title`（日程）和 `habits.name`（习惯）。
- **Modification Step 27.7：清理废弃的 `schedule_items.is_completed` 字段**已完成。
- 从 Drizzle schema 和数据库表移除已废弃的 `is_completed` 字段，完成状态已由 `schedule_completions` 表管理。
...
### ✅ Modification Step 27.11：统一服务端和客户端循环日程判断逻辑

已完成内容：

- `scheduleOccursOnDate` 提取到 `src/lib/schedules/options.ts`，接受 `(startDate, endDate, recurrence, date)` 签名。
- 服务端 `user-data.ts` 中原有 `scheduleOccursOnDate` 函数删除，改为导入共享函数。
- 客户端 `checklist-client.tsx` 中原有内联 daily/weekly/monthly 判断逻辑删除，改为调用共享函数。
- 修正服务端原实现中 daily 循环缺少 endDate 检查的 Bug。
...
- 移动端底部导航 `BottomNav` 增加搜索按钮（搜索图标 + "搜索"标签），置于首位，与 5 个 Tab 均分排列。
- 点击搜索按钮通过 `Ctrl+K` 事件触发 `SearchOverlay`，无需状态共享改造。
...
### ✅ Modification Step 27.8：搜索扩展到日程和习惯

已完成内容：

- 全局搜索 `searchTableConfig` 新增 `schedule_items`（搜 title，日期字段 start_date）和 `habits`（搜 name，日期字段 start_date）。
- 现在搜索日程标题或习惯名称能返回结果。
...
### ✅ Modification Step 27.7：清理废弃的 `schedule_items.is_completed` 字段

已完成内容：

- 从 `src/db/schema.ts` 移除 `isCompleted` 字段定义。
- 生成迁移文件 `drizzle/0013_drop_is_completed.sql`，删除 `schedule_items.is_completed` 列。
- 更新 `drizzle/meta/_journal.json` 包含该迁移。
- 已执行真实数据库迁移。

影响文件：

- `src/db/schema.ts`
- `drizzle/0013_drop_is_completed.sql`（新建）
...
- `npm run build` 通过。
- `git diff --check` 通过。
- 应用代码已无 `is_completed`/`isCompleted` 引用。
