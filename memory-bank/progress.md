...
项目已完成 Step 3.3：任务状态更新。

当前目标：

- 保持当前基础视觉系统、基础页面、导航、Supabase client 工具层、Drizzle schema、迁移流程、认证入口、安全跳转、写入保护 helper、每日工作台结构、今日任务创建和任务状态更新能力稳定。
- 准备进入 Step 3.4：实现习惯创建。
...
- Step 3.3 已实现任务状态更新。
- 未实现习惯、日程、事件或灵感的真实读写。
- 今日概览中的任务卡已接入真实今日任务完成率，其他统计仍为静态占位。
- 未配置 Row Level Security。
