...
下一步建议新增月复盘发送预览，展示将用于 AI 的月度统计、关键记录摘要和已有周复盘摘要；用户确认前不调用 AI。
