...
- 成长记录里的事件列表和人生页保持一致，每条事件最多展示两行内容。

完成内容：

- `/records` 的近期概览在移动端改为三列紧凑网格。
- `/records` 的近期记录列表改为按北京时间日期分组展示。
- 任务、习惯、日程、事件和灵感条目继续保留原有详情页跳转。
- 事件记录标题最多展示两行，超出内容进入事件详情页查看和编辑。
- 原有类型筛选和日期范围筛选继续保留。
